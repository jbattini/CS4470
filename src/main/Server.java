...
public class Server extends Thread {
							...
							try {
								...
								if (clients.get(i).getInputStream().available() != 0) {
									...
									if (args[0].equals("terminate")) {
										int termID = Integer.parseInt(args[1]);
										terminateClient(termID);
	...
	public static void getClients() {
		...
		for (int i = 0; i < clients.size(); i++) {
			System.out.println(" " + i + "  " + clients.get(i).getInetAddress().getHostAddress() + "			"
	...
	private void sendClientsListToClient(int clientID) {
		...
		try {
			...
			for (int i = 0; i < clients.size(); i++) {
				out.writeUTF(" " + i + "  " + clients.get(i).getInetAddress().getHostAddress() + "			"
						+ clients.get(i).getPort());
			}
			out.flush();
		} catch (IOException e) {
	...
	private void terminateClient(int termID) { 
		System.out.println("Terminating Client " + termID);
	
		try {
			Socket termSocket = new Socket();
			out = new DataOutputStream(clients.get(termID).getOutputStream());
			for(int i = 0; i < clients.size(); i++) {
				if(termID == i) {
					termSocket = clients.get(i);
					clients.remove(termSocket);
					termSocket.close();
				}
			}
			out.writeUTF("You've been removed from the room");
			out.flush();
			termSocket.close();
